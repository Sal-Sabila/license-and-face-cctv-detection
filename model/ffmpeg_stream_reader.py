...
class FFmpegStreamReader:
    ...
    def __init__(self, rtmp_url, width, height, ffmpeg_path="ffmpeg"):
        ...
        # Resolve URL: jika stream Wowza H.265 di /live/, gunakan endpoint HLS agar tidak timeout
        self.stream_url = rtmp_url
        if isinstance(rtmp_url, str):
            clean_url = rtmp_url.strip()
            if clean_url.startswith("rtmp://") and ":1935/live/" in clean_url:
                self.stream_url = clean_url.replace("rtmp://", "http://")
                if not self.stream_url.endswith("/playlist.m3u8"):
                    self.stream_url = self.stream_url.rstrip("/") + "/playlist.m3u8"
